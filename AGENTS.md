...
what's pending — those two are the source of truth. `docs/ARCHITECTURE.md`
covers the three-layer library design (`common.sh` → `nsx_edge.sh`/
`nsx_manager.sh` → thin `automations/*`) and the Bash-vs-Go decision rule;
`docs/CONTRIBUTING.md` has the full "add a new automation" walkthrough and
the fan-out contract summarized below.

## Commands

```bash
bash -n path/to/script.sh                       # syntax-check a single script before committing
shellcheck -x -e SC1091,SC2155,SC2034,SC2086,SC2317 path/to/script.sh   # same excludes CI uses
bats tests/                                      # run the full test suite
bats tests/test_parsers.bats                     # run a single test file
bats tests/test_parsers.bats -f "some test name" # run a single test case
```

CI (`.github/workflows/lint.yml`) runs shellcheck + bats on every push/PR to
`main` — both must pass before merging. Only pure parsers (no SSH/network)
get bats coverage; see `tests/test_parsers.bats` for the pattern.
