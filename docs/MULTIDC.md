...
# Needed by root-using automations (e.g. apiuser_audit). --all-dcs walks each
# jump via ssh -t and runs configure_ssh_keys.sh --type manager --root there;
# you enter each DC's admin + root passwords on its own jump — nothing stored here.
./bin/configure_ssh_keys.sh --all-dcs --conf ./datacenters.conf
```
